...
export default function aStar(grid, startNode, finishNode) {
  ...
  startNode.distance = (Math.abs(Math.pow((startNode.row- finishNode.row),2))+ Math.abs(Math.pow((startNode.col- finishNode.col),2)))
...
function updateUnvisitedNieghbors(node, grid, heap, finishNode) {
  ...
  for (const neighbor of unvisitedNeighbors) {
    ...
    neighbor.endDist= (Math.abs(Math.pow((finishNode.row-neighbor.row),2)) + Math.abs(Math.pow((finishNode.col-neighbor.col),2)))
